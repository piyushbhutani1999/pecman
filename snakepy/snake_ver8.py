#snake head function and multifunctions related to snake length
import pygame
import time
import random

pygame.init()

white = (255,255,255)
black = (0,0,0)
red = (255,0,0 )
green = (0,0,255)
 
display_width = 500
display_height = 500
block_size = 20
FPS = 10
head_up = pygame.image.load("snakeHeadup.fw.png")
head_down = pygame.image.load("snakeHeaddown.fw.png")
head_right = pygame.image.load("snakeHeadright.fw.png")
head_left = pygame.image.load("snakeHeadleft.fw.png")
tailUp = pygame.image.load("tailUp.fw.png")
tailDown = pygame.image.load("tailUp.fw.png")
tailRight = pygame.image.load("tailUp.fw.png")
tailLeft = pygame.image.load("tailUp.fw.png")

##  gives message to screen
font = pygame.font.SysFont(None, 25)
## SysFont is a font type and 25 is the sixe of the font
def message(msg,color):
    screen_text = font.render(msg , True , color)
    gameDisplay.blit(screen_text, [(display_width/3)-80 , display_height/2])
    #blit function help in showing the text to screen [x and y cordinate of showing]

gameDisplay = pygame.display.set_mode((display_width,display_height))
# set the display of the game

def snake(block_size,snakeList,snakeHead,direction,changeDirection,storePointList,tail):
    
    if direction == "down":
        head = head_down
    elif direction == "up":
        head = head_up
    elif direction == "right":
        head = head_right
    elif direction == "left":
        head = head_left
        
    for x in snakeList:
        if x == snakeHead:
            gameDisplay.blit(head,(x[0],x[1]))
        elif x!= snakeList[0]:
            pygame.draw.rect(gameDisplay, black,[x[0],x[1],block_size,block_size])
        elif x == snakeList[0]:
            if len(storePointList)>0 and len(changeDirection)>0:
                if snakeList[0]==storePointList[0]:
                    if changeDirection[0] == "down":
                        tail = tailDown
                    elif changeDirection[0] == "up":
                        tail = tailUp
                    elif changeDirection[0] == "right":
                        tail = tailRight
                    elif changeDirection[0] == "left":
                        tail = tailLeft
                    gameDisplay.blit(tail,(x[0],x[1]))
                    del storePointList[0]
                    del changeDirection[0]
                    
                else :
                    gameDisplay.blit(tail,(x[0],x[1]))
            else :
                 gameDisplay.blit(tail,(x[0],x[1]))



pygame.display.set_caption("Feed The Snake")
# set the caption of the game

clock = pygame.time.Clock()
# return us a pygame clock object

def gameLoop():
    gameExit = False
    gameOver = False
    temp=0
    snakeLength = 3
    tail = tailRight
    direction = "right"
    lead_x = (display_width/2)-10
    lead_y = (display_width/2)-10
    lead_x_change = 10
    lead_y_change = 0
    feed_x = random.randint(0, (display_width/block_size)-1)
    feed_x = feed_x*block_size
    feed_y = random.randint(0, (display_height/block_size)-1)
    feed_y = feed_y*block_size
    print(f"feed_x={feed_x} and feed_y={feed_y}")
    snakeList= []
    changeDirection = []
    storePoint = []
    storePointList = []
    while not gameExit:
        while gameOver == True:
            gameDisplay.fill(white)
            message("Press A to -PLAY AGAIN- and X to -QUIT-",red)
            pygame.display.update()

            for event in pygame.event.get():
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_a:
                        gameLoop()
                    elif event.key == pygame.K_x:
                        gameExit = True
                        gameOver = False

        for event in pygame.event.get():
            if event.type == pygame.QUIT:
               gameExit = True
            if event.type == pygame.KEYDOWN:
                storePoint= []
                if event.key == pygame.K_LEFT:
                    if temp == 276 or temp == 275:
                        continue
                    lead_x_change = -block_size
                    lead_y_change = 0
                    direction = "left"
                    changeDirection.append(direction)
                    storePoint.append(lead_x + lead_x_change)
                    storePoint.append(lead_y + lead_y_change)
                elif event.key == pygame.K_RIGHT:
                    if temp == 276 or temp == 275:
                        continue
                    lead_x_change = block_size
                    lead_y_change = 0
                    direction = "right"
                elif event.key == pygame.K_UP:
                    if temp == 273 or temp == 274:
                        continue
                    lead_y_change = -block_size
                    lead_x_change = 0
                    direction = "up"
                elif event.key == pygame.K_DOWN:
                    if temp == 273 or temp == 274:
                        continue
                    lead_y_change = block_size
                    lead_x_change = 0
                    direction = "down"
                changeDirection.append(direction)
                storePoint.append(lead_x + lead_x_change)
                storePoint.append(lead_y + lead_y_change)
                storePointList.append(storePoint)
                print(f"list = {storePointList} direction is {changeDirection}")
                temp = event.key
            

        lead_x = lead_x + lead_x_change
        lead_y = lead_y + lead_y_change
        if lead_x >= display_width or lead_x<0 or lead_y >= display_height or lead_y<0:
            gameOver = True
        if lead_x == feed_x and lead_y ==feed_y:
            feed= True
            while feed:
                feed_x = random.randint(0, (display_width/block_size)-1)
                feed_x = feed_x*block_size
                feed_y = random.randint(0, (display_height/block_size)-1)
                feed_y = feed_y*block_size
                snakeLength = snakeLength + 1
                for x in snakeList:
                    if x[0]!=feed_x or x[1]!=feed_y:
                        feed = False
                
            
        gameDisplay.fill(white)
        snakeHead = []
        snakeHead.append(lead_x)
        snakeHead.append(lead_y)
        snakeList.append(snakeHead)
        if len(snakeList)>snakeLength:
            del snakeList[0]
        if snakeLength>3:
            for x in snakeList[:-1]:
                if x == snakeHead:
                    gameOver= True
                
        snake(block_size,snakeList,snakeHead,direction,changeDirection,storePointList,tail)
        pygame.draw.rect(gameDisplay,red,[feed_x,feed_y,block_size,block_size])
        clock.tick(FPS)
        pygame.display.update()
        
    pygame.quit()
    quit()
gameLoop()
